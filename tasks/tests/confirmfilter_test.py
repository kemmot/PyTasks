...
class ConfirmFilterTests(unittest.TestCase):
    def test_constructor_sets_action_name(self):
        ...
        confirm_filter = confirmfilter.ConfirmFilter(mock_context, action_name)
        self.assertEqual(action_name, confirm_filter.action_name)

    def test_filter_items_does_not_prompt_for_zero_items(self):
        mock_context = self.create_mock_context('no')
        items = []
        confirm_filter = confirmfilter.ConfirmFilter(mock_context, '')
        confirm_filter.filter_items(items)
    ...
    def test_filter_items_prompts_for_one_item(self):
        ...
        mock_item.name = 'test'
        items = [mock_item]
        confirm_filter = confirmfilter.ConfirmFilter(mock_context, 'action')
        confirm_filter.filter_items(items)
    ...
    def test_filter_items_returns_empty_list_on_negative_confirmation(self):
        confirm_filter = confirmfilter.ConfirmFilter(self.create_mock_context('no'), 'action')
        items = [mock.Mock(), mock.Mock()]
        result = confirm_filter.filter_items(items)
        self.assertEqual(0, len(result))

    def test_filter_items_returns_original_list_on_positive_confirmation(self):
        confirm_filter = confirmfilter.ConfirmFilter(self.create_mock_context('yes'), 'action')
        items = [mock.Mock(), mock.Mock()]
        result = confirm_filter.filter_items(items)
